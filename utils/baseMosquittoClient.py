...
from setting.setting import QOSLOCAL
from utils.mosquittoReceiveMessage import MosquittoReceiveMessage
from utils.publishMessage import PublishMessage


class BaseMosquittoClient():
    def __init__(self,host='127.0.0.1',port=1883,username='test001',password='test001',keepalive=60,bind_address='',ioloop=None,aioloop=None):
        self.host = host
        self.port = port
    ...
    def on_message(self,client, userdata, message):
        ReceiveMessageObject = MosquittoReceiveMessage(client, userdata, message)
        self.ReceiveBuffer.append(ReceiveMessageObject)
    ...
    def handle_on_message(self):
        while self.ReceiveBuffer:
            try:
                mosquittoReceiveMessageObject = self.ReceiveBuffer.pop(0)
                yield self.handleOnMessage(mosquittoReceiveMessageObject)
            ...
            self.handle_message_task = None
    @gen.coroutine
    def handleOnMessage(self,mosquittoReceiveMessageObject):
        pass
    ...
    def myPublish(self, topic, payload=None, qos=0, retain=False):
        publishMessage = PublishMessage(topic=topic,payload=payload,qos=qos,retain=retain)
